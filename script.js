...
const anniversaryData = {
  ...
  memories: [
    ["How we met", "[DATE/LABEL]", "CUSTOMIZE: Write the moment your story began.", ["assets/photos/memory-1.svg"], "☁️"],
    ["Our first date", "[DATE/LABEL]", "CUSTOMIZE: Add the tiny details you still remember.", ["assets/photos/memory-2.svg"], "🍵"],
    ["Our funniest memory", "[DATE/LABEL]", "CUSTOMIZE: The joke that still makes you both laugh.", ["assets/photos/memory-3.svg"], "🐵"],
    ["A favorite meal together", "[DATE/LABEL]", "CUSTOMIZE: Tell the story of a food date.", ["assets/photos/memory-4.svg"], "🥖"],
    ["A favorite trip", "[DATE/LABEL]", "CUSTOMIZE: Describe the place that felt like an adventure.", ["assets/photos/memory-5.svg"], "⛵"],
    ["A quiet everyday moment", "[DATE/LABEL]", "CUSTOMIZE: Celebrate the ordinary comfort.", ["assets/photos/memory-6.svg"], "🦆"],
    ["A difficult moment we overcame", "[DATE/LABEL]", "CUSTOMIZE: Honor how you chose each other.", ["assets/photos/memory-7.svg"], "💌"],
    ["What I love about her", "Always", "CUSTOMIZE: Be specific, soft, and honest.", ["assets/photos/memory-8.svg"], "✨"],
    ["My hopes for our future", "Tomorrow + after", "CUSTOMIZE: Dream out loud here.", ["assets/photos/memory-9.svg"], "🗺️"]
  ],
  photoStrips: [
    { caption: "Four tiny frames, one big feeling.", photos: ["assets/photos/strip-1a.svg","assets/photos/strip-1b.svg","assets/photos/strip-1c.svg","assets/photos/strip-1d.svg"] },
    { caption: "Proof that ordinary days are magic.", photos: ["assets/photos/strip-2a.svg","assets/photos/strip-2b.svg","assets/photos/strip-2c.svg","assets/photos/strip-2d.svg"] },
    { caption: "The booth kept our favorite smiles.", photos: ["assets/photos/strip-3a.svg","assets/photos/strip-3b.svg","assets/photos/strip-3c.svg","assets/photos/strip-3d.svg"] }
  ],
  ...
  easterEggMessages: { monkey: "You found the cheeky monkey. I love how much you make me laugh.", ducks: "Two little ducks, one very big love.", strawHat: "Secret photo unlocked! Replace this with a silly favorite picture.", mapX: "CUSTOMIZE: Put your longer private love letter in this hidden treasure-map note." }
};
...
function buildReasons(){ $('#reasonSky').innerHTML = anniversaryData.reasonsILoveYou.map((r,i)=>`<button class="reason" type="button">${i%2?'⭐':'☁️'} <span>${r}</span></button>`).join(''); }
...
function bind(){ $('#openBooth').addEventListener('click',openBooth); $('#replay').addEventListener('click',()=>{window.scrollTo({top:0,behavior:reduced?'auto':'smooth'}); $('#intro').classList.remove('is-open'); document.body.classList.remove('ready'); introAnimations();}); $('#closeModal').onclick=()=>$('#modal').hidden=true; $('#modal').addEventListener('click',e=>{if(e.target.id==='modal')$('#modal').hidden=true}); addEventListener('keydown',e=>{ if(e.key==='Escape')$('#modal').hidden=true; loveCode(e.key); });
  ...
  let hat=0; $('#strawHat').onclick=()=>{ if(++hat===3) modal('Secret photo', `<img src="assets/photos/secret-placeholder.svg" alt="CUSTOMIZE: secret photo" style="max-width:100%;border-radius:16px"><p>${anniversaryData.easterEggMessages.strawHat}</p>`); };
